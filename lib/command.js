...
Meteor.command = {
    ...
    insert: function (after) {
        var command = Meteor.command.createCommand('insert', null, after,
            function (before, after) {
                //exec
                var insertId = Meteor.spitfire.uid();
                if (Object.prototype.toString.call(after) === '[object Array]' && after.length > 0) {
                    for (var i = 0; i < after.length; i++) {
                        after[i].initId = insertId;
            ...
            function (before, after) {
                //unexec
                if (Object.prototype.toString.call(after) === '[object Array]' && after.length > 0) {
                    var bulkInsert = DrawingObjects.find({initId: after[0].initId});
