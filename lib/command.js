            ...
            function (before, after) {
                ...
                if (Meteor.util.isArray(after) && after.length > 0) {
                    for (var i = 0; i < after.length; i++) {
                        ...
                        //make sure that fatherIds do not interfere with already stored content
                        after[i]._id = bulkId + after[i]._id;
                        if (after[i].fatherId) {
                            after[i].fatherId = bulkId + after[i].fatherId;
                        }

                        Meteor.call("insert", after[i], function (error, insertResult) {
                            if (insertResult.givenId) {
                                //keep track of changed ids in command structure
                                Meteor.command._adaptIdInCommands(insertResult.givenId, insertResult._id);
                                //keep track of changed father ids in database
                                insertedIds.push(insertResult);

                                if (insertedIds.length === after.length) {
                                    //use timeout to ensure the data is really there
                                    setTimeout(function () {
                                        for (var j = 0; j < insertedIds.length; j++) {
                                            var oldFatherId = insertedIds[j].givenId;
                                            var newFatherId = insertedIds[j]._id;
                                            Meteor.call("reConnectById", oldFatherId, newFatherId);
                                        }
                                    }, 1500);
                                }
                            }
                ...
                if (before) {
                    Meteor.call("insert", before,
                        function (error, result) {
                            ...
                            Meteor.command._adaptIdInCommands(result.givenId, result._id);
                        });
                }
            }
        );
            ...
            function (before, after) {
                ...
                if (one) {
                    ...
                    Meteor.call("insert", after, function (error, id) {
                        Meteor.command._adaptIdInCommands(after._id, id);
                    });
                }
            },
            function (before, after) {
