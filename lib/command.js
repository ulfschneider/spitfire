    ...
    adaptId: function (from, to) {
        for (var i = 0; i < commands.length; i++) {
            ...
            if (commands[i].before) {
                if (Meteor.spitfire.isArray(commands[i].before)) {
            ...
            if (commands[i].after) {
                if (Meteor.spitfire.isArray(commands[i].after)) {
                    for (var j = 0; j < commands[i].after.length; j++) {
    ...
    insert: function (after) {
        var command = Meteor.command.createCommand("insert", null, after,
            function (before, after) {
                ...
                if (Meteor.spitfire.isArray(after) && after.length > 0) {
