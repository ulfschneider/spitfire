...
Meteor.command = {
    ...
    _decorateId: function (id, decoration, type) {
        if (id) {
            ...
            function (before, after) {
                ...
                if (Meteor.util.isArray(after) && after.length > 0) {
                    ...
                    for (var i = 0; i < after.length; i++) {
                        ...
                        after[i]._id = Meteor.command._decorateId(after[i]._id, bulkId, "bulk");
                        after[i].fatherId = Meteor.command._decorateId(after[i].fatherId, bulkId, "bulk");

                        Meteor.call("insert", after[i], true);
                    }
                }
            },
            function (before, after) {
