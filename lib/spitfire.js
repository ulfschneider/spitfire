...
Meteor.spitfire = {
    ...
    prepareConnect: function (source, destination) {
        if (source.next) {
...
(function () {
    Meteor.methods({
            ...
            connect: function (predecessor, successor) {
                if (successor.next == predecessor ||  successor._id == predecessor) {
                    var succUpdate = {
                        next: null,
                        modifiedAt: new Date(),
                        user: Meteor.spitfire.getUser()
                    };
                    Meteor.call("maintainSessionData", succUpdate);
                    DrawingObjects.update(successor._id, {
                        $set: succUpdate
                    });
                }

                if (predecessor !== successor._id) {
                    var predUpdate = {
                        next: successor._id,
                        modifiedAt: new Date(),
                        user: Meteor.spitfire.getUser()
                    };
                    Meteor.call("maintainSessionData", predUpdate);
                    return DrawingObjects.update(predecessor, {
                        $set: predUpdate
                    });
                }
