...
function initRouter() {
    Router.route('/', 'editor');
    Router.route('/about');
    Router.route('/:sessionName/:grid?/:any2?/:any3?', function () {
        ...
        if (grid) {
            var x = grid.toLowerCase().indexOf('x');
            if (x < 0) {
                Meteor.grid.setXGrid(parseInt(grid));
            } else {
                var xgrid = grid.substring(0, x);
                var ygrid = grid.substring(x + 1, grid.length);
                Meteor.grid.setXGrid(parseInt(xgrid));
                Meteor.grid.setYGrid(parseInt(ygrid));
            }
        }
        Meteor.spitfire.setSessionName(sessionName);
        if (grid) {
            Router.go('/' + sessionName + '/' + grid);
        } else {
...
Meteor.spitfire = {
    ...
    getRegExFilter: function () {
        var filter = Meteor.spitfire.getFilter();
