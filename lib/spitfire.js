...
Meteor.spitfire = {
    ...
    hasDrawingObjects:function() {
        return DrawingObjects.find().count() > 0;
    },
    ...
    initServer: function () {
        ...
        Router.route("/:sessionSpecifier/export", function () {
            ...
            var fileContent = Meteor.export.createExportData(sessionName);
...
(function () {
    Meteor.methods({
            ...
            connectById: function (sonId, fatherId) {
                ...
                DrawingObjects.update(sonId, {
                    $set: update
                });
            },
            unConnectById: function (sonId) {
                var update = {
                    modifiedAt: new Date(),
                    user: Meteor.spitfire.getUser(),
                    fatherId: null
            ...
            removeById: function (id) {
                ...
                if (Meteor.util.isArray((id))) {
                    for (var i = 0; i < id.length; i++) {
                        data = Meteor.spitfire.loadDrawingObject(id[0]);
                        if (data) {
